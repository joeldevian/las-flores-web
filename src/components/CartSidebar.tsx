    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-md flex flex-col h-full shadow-2xl overflow-hidden" style={{ background: R.crema }}>
          ...
          <div className="flex items-center py-4 px-5 relative h-16">
            ...
            <div className="absolute right-5 flex items-center gap-4">
              <div className="relative hidden sm:block">
                ...
                {totalItems > 0 && (
                  <span className="absolute -top-1 -right-2 w-[18px] h-[18px] rounded-full flex items-center justify-center text-[10px] font-bold shadow-sm" style={{ background: R.rojo, color: "white" }}>
                    {totalItems}
                  </span>
                )}
              </div>
        ...
        {step !== "success" && (
          <div className="flex flex-shrink-0 bg-white border-b border-black/5 shadow-sm">
            {(["cart","delivery","payment"] as const).map((s, i) => {
              ...
              return (
                <div key={s} className="flex-1 py-3 flex flex-col items-center gap-1 text-[10px] font-bold tracking-wider uppercase transition-all"
                  style={{ borderBottom: isActive ? `3px solid ${R.amarillo}` : "3px solid transparent" }}
                >
                  <div className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold transition-all shadow-sm"
                    style={{
                  ...
                  <span style={{ color: isActive ? R.morado : isPast ? R.morado : "rgba(0,0,0,0.3)" }}>
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            <div className="p-5 min-h-full">
              {items.length === 0 ? (
                <div className="text-center py-24 px-6 flex flex-col items-center">
                  <div className="relative w-24 h-32 mb-6 flex flex-col items-center justify-center rounded-t-full rounded-b-xl border-[3px] shadow-sm" style={{ borderColor: R.morado, background: `${R.amarillo}15` }}>
                    <div className="absolute inset-1.5 rounded-t-full rounded-b-lg border-2 border-dashed" style={{ borderColor: `${R.rojo}60` }} />
                    <ShoppingBag size={36} style={{ color: R.rojo }} className="relative z-10" />
                  </div>
                  <p className="font-serif text-xl font-bold text-black/80">Tu canasta está vacía</p>
                  <p className="text-sm mt-3 text-black/45 leading-relaxed">
                    Las puertas de nuestro retablo están abiertas. Explora la carta y añade delicias.
                  </p>
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                          <div>
                            <p className="font-serif text-sm font-bold leading-snug text-black/85">{item.name}</p>
                            <p className="text-sm font-bold mt-1" style={{ color: R.rojo }}>S/ {item.price.toFixed(2)}</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <button onClick={() => updateQuantity(item.id, item.quantity - 1)}
                              ...
                              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = R.rojo; (e.currentTarget as HTMLElement).style.color = "white"; }}
                              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "transparent"; (e.currentTarget as HTMLElement).style.color = R.rojo; }}
                            >
                              <Minus size={11} strokeWidth={3} />
                            </button>
                            <span className="text-sm font-bold w-5 text-center text-black/70">{item.quantity}</span>
                            <button onClick={() => updateQuantity(item.id, item.quantity + 1)}
                            ...
                            <button onClick={() => removeItem(item.id)} className="ml-auto opacity-20 hover:opacity-50 transition-opacity">
                              <Trash2 size={14} />
                            </button>
                          </div>
                        </div>
              ...
              {items.length > 0 && (
                <div className="mt-6">
                  <p className="text-[10px] uppercase tracking-[0.14em] font-bold text-black/40 mb-3">Tipo de pedido</p>

                  <div className="grid grid-cols-2 gap-3 mb-5">
                    {([
                      { type:"delivery" as OrderType, label:"Delivery",    sub:"A tu puerta",   Icon:Truck, color:R.rojo },
                      { type:"pickup"   as OrderType, label:"Para Llevar", sub:"En restaurante", Icon:Store, color:R.verde },
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="block text-[10px] mt-0.5 opacity-70 font-sans">{sub}</span>
                        </button>
                      );
                    })}
                  </div>
                  ...
                  <div className="bg-white rounded-xl p-4 border border-black/5 shadow-sm space-y-2 text-sm">
                    <div className="flex justify-between text-black/50 font-medium">
                      <span>Subtotal ({totalItems} {totalItems === 1 ? "plato" : "platos"})</span>
          ...
          {/* PASO 2: ENTREGA */}
          {step === "delivery" && (
            <form id="delivery-form" className="p-5 space-y-4 min-h-full"
              onSubmit={(e) => { e.preventDefault(); setStep("payment"); }}>

              {orderType === "delivery" && (
                <div className="rounded-xl p-3.5 flex gap-2.5 text-sm"
                  style={{ background: `${R.amarillo}15`, border: `1px solid ${R.amarillo}50`, color: R.morado }}>
                  <MapPin size={15} className="flex-shrink-0 mt-0.5" />
                  <span>Solo en Huamanga, Ayacucho — <strong>30 minutos estimados</strong></span>
                </div>
              )}
              ...
              {!delivery.email ? (
                <div className="py-10 text-center px-6 bg-white rounded-xl border border-black/5 shadow-sm">
                  ...
                  <button type="button"
                    onClick={() => setDelivery({ ...delivery, name:"Luis Llocclla", email:"luis@ejemplo.com" })}
                ...
                <div className="space-y-4 animate-in fade-in duration-300">
                  <div className="flex items-center gap-2.5 p-3 rounded-xl text-sm font-medium border"
                    style={{ background:`${R.verde}10`, borderColor:`${R.verde}30`, color: R.verde }}>
                    <CheckCircle size={16} />
                  ...
                  <div>
                    ...
                    <input required value={delivery.name} readOnly
                      className={`${inputCls} bg-black/5 cursor-not-allowed text-black/40`} />
                  </div>
                  <div>
                    <Label>Teléfono / Celular *</Label>
                    <input required value={delivery.phone}
                  ...
                  {orderType === "delivery" && (
                    <>
                      <div>
                        ...
                        <input required value={delivery.address}
                          onChange={(e) => setDelivery({ ...delivery, address: e.target.value })}
                          placeholder="Av. Principal 123, Dpto 201" className={inputCls} />
                      </div>
                      <div>
                        <Label>Referencia</Label>
                        <input value={delivery.reference}
                          onChange={(e) => setDelivery({ ...delivery, reference: e.target.value })}
                          placeholder="Frente al parque, edificio blanco..." className={inputCls} />
                      </div>
                      <div>
          ...
          {step === "payment" && (
            <form id="payment-form" className="p-5 space-y-4 min-h-full" onSubmit={handlePayment}>
              ...
              {paymentMethod === "yape" ? (
                <div className="bg-white rounded-xl p-5 border shadow-sm space-y-4" style={{ borderColor: `${R.amarillo}50` }}>
                  <div className="flex flex-col items-center">
                    <div className="w-36 h-36 rounded-xl p-3 mb-3 border bg-white" style={{ borderColor: `${R.amarillo}50` }}>
                      <div className="w-full h-full rounded-lg"
                        style={{ backgroundImage:`repeating-linear-gradient(0deg,${R.morado} 0,${R.morado} 2px,transparent 2px,transparent 10px),repeating-linear-gradient(90deg,${R.morado} 0,${R.morado} 2px,transparent 2px,transparent 10px)` }}
                      />
                    </div>
                    <p className="font-serif font-bold text-sm text-black/80">Escanea con Yape o Plin</p>
                    <p className="text-xs text-black/50 mt-0.5">A nombre de <strong>Las Flores SAC</strong></p>
                  </div>
                  <div>
                ...
                <div className="bg-white rounded-xl p-5 border border-[#0057A8]/15 shadow-sm space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input required value={payment.cardNumber}
                        ...
                        placeholder="0000 0000 0000 0000" maxLength={19} className={`${inputCls} pr-10`} />
                      <CreditCard size={15} className="absolute right-3 top-1/2 -translate-y-1/2 text-black/20" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input required value={payment.expiry}
                        ...
                        placeholder="MM/AA" maxLength={5} className={inputCls} />
                    </div>
                    <div>
                      <Label>CVV *</Label>
                      <input required type="password" value={payment.cvv}
                  ...
                  <div>
                    ...
                    <input required value={payment.cardName}
                      onChange={(e) => setPayment({ ...payment, cardName: e.target.value.toUpperCase() })}
                      placeholder="JUAN PEREZ" className={`${inputCls} uppercase`} />
                  </div>
                </div>
              )}

              <div className="bg-white rounded-xl p-4 border border-black/5 text-sm space-y-1.5 shadow-sm">
                <div className="flex justify-between text-black/50 font-medium"><span>Subtotal</span><span>S/ {totalPrice.toFixed(2)}</span></div>
                {orderType === "delivery" && (
                  <div className="flex justify-between text-black/50 font-medium"><span>Delivery</span><span>S/ {DELIVERY_FEE.toFixed(2)}</span></div>
                )}
                <div className="flex justify-between font-serif font-bold text-base pt-2 border-t border-black/5">
                  <span className="text-black/70">Total</span>
                  <span style={{ color: R.rojo }}>S/ {total.toFixed(2)}</span>
          ...
          {step === "success" && (
            <div className="p-8 flex flex-col items-center text-center min-h-full justify-center">
              ...
              <h3 className="text-2xl font-serif font-bold mb-1" style={{ color: R.morado }}>Pedido confirmado</h3>
              <p className="text-black/60 text-sm mb-1 font-medium">Tu pedido ha sido recibido.</p>
              {orderType === "delivery" ? (
                <p className="text-black/50 text-xs mb-6">
                  Llegará a <strong className="text-black/70">{delivery.address || "tu dirección"}</strong> en aprox. <strong style={{ color: R.verde }}>30 min</strong>.
                </p>
              ) : (
                <p className="text-black/50 text-xs mb-6">
              ...
              <div className="w-full bg-white rounded-xl p-4 border border-black/5 text-left mb-4 shadow-sm">
                <p className="text-[10px] uppercase tracking-[0.12em] font-bold text-black/40 mb-3">Resumen</p>
                {items.map((item) => (
                  <div key={item.id} className="flex justify-between text-sm text-black/60 font-medium mb-1.5">
              ...
              <p className="text-xs text-black/30 font-medium">
                Pedido <strong style={{ color: R.morado }}>#LF{Date.now().toString().slice(-6)}</strong>
              </p>
            </div>
          )}
        </div>
