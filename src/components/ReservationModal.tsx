      ...
      <div className="relative w-full max-w-[450px] bg-[#f8f4e6] h-[95%] max-h-[850px] shadow-2xl flex flex-col animate-in zoom-in-95 duration-500 overflow-hidden rounded-2xl md:rounded-3xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-ink/10 bg-white/50 backdrop-blur-md z-10">
          ...
          <img
            src="/images.png"
            alt="Logo"
            className="h-10 object-contain drop-shadow-sm scale-[1.35] origin-center"
          />
          <button
            onClick={onClose}
            className="p-2 hover:bg-black/5 rounded-full transition-colors"
                ...
                <div className="mt-2 flex justify-center">
                  ...
                  >
                    ...
                    {form.date && !DATES.find((d) => d.value === form.date)
                      ? new Date(form.date + "T12:00").toLocaleDateString("es-PE", {
                          weekday: "short",
                          day: "numeric",
                          month: "long",
                        })
            ...
            {step === 4 && (
              <div className="flex flex-col h-full items-center justify-center animate-in fade-in zoom-in-95 duration-500">
                {!form.email ? (
                  <>
                    <div className="text-center mb-8">
                      <h2 className="font-serif text-3xl font-bold text-ink mb-2">Casi listo</h2>
                      <p className="text-xs text-ink/70 px-4">
                  ...
                  <div className="text-center w-full space-y-6">
                    <div className="p-4 bg-white/90 rounded-2xl border border-eucalipto/20 shadow-xs text-center">
                      <span className="block text-xs text-ink/60 uppercase tracking-wider font-bold mb-1">Identificado como</span>
                      <span className="block font-serif font-bold text-lg text-eucalipto">{form.name}</span>
                      <span className="block text-xs text-ink/70 mt-0.5">{form.email}</span>
                    </div>

                    <button
                      onClick={() => setStep(5)}
                      className="w-full py-4 rounded-xl font-bold uppercase tracking-wider transition-all bg-eucalipto text-cream shadow-md hover:shadow-lg hover:-translate-y-0.5"
                    >
                      Seleccionar mesa en el mapa &rarr;
                    </button>
                  </div>
                )}
              </div>
                ...
                <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 text-sm text-left w-full mt-8 shadow-sm border border-ink/10">
                  ...
                  <div className="space-y-3 font-medium text-ink">
                    <p className="flex justify-between border-b border-ink/10 pb-2">
                      ...
                      <strong>
                        {form.table === "Aleatoria" ? "Asignación a la llegada" : form.table}
                      </strong>
                    </p>
                    <p className="flex justify-between border-b border-ink/10 pb-2">
